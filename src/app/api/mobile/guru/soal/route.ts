...
export async function GET(req:Request){
  try{
    ...
    const soals=await prisma.soal.findMany({where:{guruId:guru.id, deletedAt:null}, select:{id:true,pertanyaan:true,jenisSoal:true,tingkatKesulitan:true,bab:true,poin:true, mataPelajaran:{select:{nama:true}}}, orderBy:{createdAt:"desc"}, take:50});

...
export async function GET(req:Request){
  try{
    ...
    const where: Record<string, unknown> = { kelasId:{in:ids}, deletedAt:null }
    if (search) {
    ...
    const siswa=await prisma.siswa.findMany({where: where as never, select:{id:true,nama:true,nis:true, kelas:{select:{nama:true}}}, orderBy:{nama:"asc"}, take:100});

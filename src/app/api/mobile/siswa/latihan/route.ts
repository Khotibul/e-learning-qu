...
export async function GET(req:Request){
  try{
    ...
    const latihans=await prisma.ujian.findMany({where:{kelasId:siswa.kelasId, isLatihan:true, status:"AKTIF", deletedAt:null}, select:{id:true,nama:true,jumlahSoal:true,durasi:true, mataPelajaran:{select:{nama:true}}, _count:{select:{jawabanUjian:{where:{siswaId:siswa.id}}}}}, orderBy:{createdAt:"desc"}, take:50});
    return NextResponse.json(latihans.map(l=>({id:l.id, nama:l.nama, mapel:l.mataPelajaran.nama, jumlahSoal:l.jumlahSoal, durasi:l.durasi, sudahDikerjakan:l._count.jawabanUjian>0})),{headers:corsHeaders});

...
export async function GET(req:Request){
  try{
    ...
    if(!siswa?.kelasId) return NextResponse.json({error:"No class"},{status:404, headers:corsHeaders});
    const kelas=await prisma.kelas.findUnique({where:{id:siswa.kelasId}, include:{guru:{select:{nama:true}}, siswas:{where:{deletedAt:null}, select:{id:true,nama:true,jabatan:true}, orderBy:{nama:"asc"}}}});

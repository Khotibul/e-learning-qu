import { NextResponse } from "next/server"; import { getMobileUser } from "@/lib/mobile-auth"; import { prisma } from "@/lib/prisma"; import { runTutorAgent } from "@/lib/agents/tutor";
...
export async function POST(req:Request){
  try{
    ...
    const {message, mapelId} = await req.json();
    ...
    const result=await runTutorAgent(message, {mapelId: mapelId ?? null, kelasId: siswa?.kelasId ?? null, studentId: siswa?.id});

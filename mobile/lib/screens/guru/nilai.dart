...
class _GuruNilaiState extends State<GuruNilai> {
  ...
  @override void initState(){ super.initState(); ApiService.get("/api/mobile/guru/nilai").then((v){ if(mounted) setState(()=>{ ujians=v["ujians"]??[]; nilais=v["nilais"]??[]; loading=false;}); }).catchError((_){ if(mounted) setState(()=> loading=false);}); }
